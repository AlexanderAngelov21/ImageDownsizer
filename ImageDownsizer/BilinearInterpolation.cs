...
namespace ImageDownsizer_Homework
{
    public class BilinearInterpolation
    {
        public static byte[,,] ResizeImage(byte[,,] originalImage, int originalWidth, int originalHeight, int newWidth, int newHeight)
        {
            ...
            for (int x = 0; x < newWidth; x++)
            {
                for (int y = 0; y < newHeight; y++)
                {
                    ...
                    {
                        resizedImage[x, y, channel] = BilinearInterpolations(originalImage, sourceX, sourceY, channel, originalWidth, originalHeight);
                    }
                }
            }
        ...
        private static byte BilinearInterpolations(byte[,,] image, float x, float y, int channel, int width, int height)
        {
            int x1 = (int)x;
            int y1 = (int)y;
